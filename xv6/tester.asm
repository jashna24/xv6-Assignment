...
int main(int argc, char *argv[]){
  ...
  2d:	7e 2a                	jle    59 <main+0x59>
            ppid = fork();
  2f:	e8 f6 02 00 00       	call   32a <fork>
            if(ppid == 0){
  34:	85 c0                	test   %eax,%eax
  36:	74 4f                	je     87 <main+0x87>
  ...
  38:	79 ee                	jns    28 <main+0x28>
                printf(1, "%d failed in fork!\n", getpid());
  3a:	e8 83 03 00 00       	call   3c2 <getpid>
  ...
  52:	83 c4 10             	add    $0x10,%esp
        for(int j = 0; j < child; j++){
  55:	39 de                	cmp    %ebx,%esi
  57:	7f d6                	jg     2f <main+0x2f>
            }
        }
        for(int i = 0; i < child; i++){
  ...
  60:	83 c3 01             	add    $0x1,%ebx
            wait();
  63:	e8 d2 02 00 00       	call   33a <wait>
        for(int i = 0; i < child; i++){
  68:	39 de                	cmp    %ebx,%esi
  6a:	75 f4                	jne    60 <main+0x60>
                exit();
  6c:	e8 c1 02 00 00       	call   332 <exit>
        child = atoi(argv[1]);
  71:	83 ec 0c             	sub    $0xc,%esp
  74:	ff 70 04             	pushl  0x4(%eax)
  77:	e8 44 02 00 00       	call   2c0 <atoi>
        for(int j = 0; j < child; j++){
  ...
  81:	89 c6                	mov    %eax,%esi
        for(int j = 0; j < child; j++){
  83:	7f 9b                	jg     20 <main+0x20>
  85:	eb e5                	jmp    6c <main+0x6c>
                int pid = getpid();
  87:	e8 36 03 00 00       	call   3c2 <getpid>
                for (z = 0; z < limit/2; z += 1)
  8c:	d9 ee                	fldz   
                int pid = getpid();
  8e:	89 c1                	mov    %eax,%ecx
                for (z = 0; z < limit/2; z += 1)
  90:	d8 05 fc 07 00 00    	fadds  0x7fc
  96:	d9 05 00 08 00 00    	flds   0x800
  9c:	df e9                	fucomip %st(1),%st
  9e:	77 f0                	ja     90 <main+0x90>
  a0:	dd d8                	fstp   %st(0)
                set_priority(pid, 100 - pid/2);
  a2:	50                   	push   %eax
  a3:	50                   	push   %eax
  a4:	89 c8                	mov    %ecx,%eax
  a6:	bb 02 00 00 00       	mov    $0x2,%ebx
  ab:	99                   	cltd   
  ac:	f7 fb                	idiv   %ebx
  ae:	f7 d8                	neg    %eax
  b0:	83 c0 64             	add    $0x64,%eax
  b3:	50                   	push   %eax
  b4:	51                   	push   %ecx
  b5:	e8 18 03 00 00       	call   3d2 <set_priority>
                for (z = 0; z < limit/2; z += 1)
  ba:	d9 ee                	fldz   
                set_priority(pid, 100 - pid/2);
  ...
  bf:	90                   	nop
                for (z = 0; z < limit/2; z += 1)
  c0:	d8 05 fc 07 00 00    	fadds  0x7fc
  c6:	d9 05 00 08 00 00    	flds   0x800
  cc:	df e9                	fucomip %st(1),%st
  ce:	77 f0                	ja     c0 <main+0xc0>
  d0:	dd d8                	fstp   %st(0)
  d2:	eb 98                	jmp    6c <main+0x6c>
  d4:	66 90                	xchg   %ax,%ax
  d6:	66 90                	xchg   %ax,%ax
  d8:	66 90                	xchg   %ax,%ax
  da:	66 90                	xchg   %ax,%ax
 ...
 3f9:	85 d2                	test   %edx,%edx
{
 ...
 427:	f7 f1                	div    %ecx
 429:	0f b6 92 0c 08 00 00 	movzbl 0x80c(%edx),%edx
  }while((x /= base) != 0);
...
printf(int fd, const char *fmt, ...)
{
 ...
 644:	e9 8f fe ff ff       	jmp    4d8 <printf+0x48>
          s = "(null)";
 649:	bb 04 08 00 00       	mov    $0x804,%ebx
        while(*s != 0){
...
free(void *ap)
{
 ...
 661:	a1 b8 0a 00 00       	mov    0xab8,%eax
{
 ...
 69d:	89 08                	mov    %ecx,(%eax)
  freep = p;
 69f:	a3 b8 0a 00 00       	mov    %eax,0xab8
}
 6a4:	5b                   	pop    %ebx
 6a5:	5e                   	pop    %esi
 6a6:	5f                   	pop    %edi
 ...
 6b6:	72 ce                	jb     686 <free+0x26>
{
 ...
 6d7:	03 53 fc             	add    -0x4(%ebx),%edx
  freep = p;
 6da:	a3 b8 0a 00 00       	mov    %eax,0xab8
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 6f9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6fc:	8b 15 b8 0a 00 00    	mov    0xab8,%edx
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 794:	89 78 04             	mov    %edi,0x4(%eax)
      freep = prevp;
 797:	89 15 b8 0a 00 00    	mov    %edx,0xab8
}
 ...
 7b7:	0a 00 00 
 7ba:	c7 05 bc 0a 00 00 bc 	movl   $0xabc,0xabc
 7c1:	0a 00 00 
    base.s.size = 0;
 7c4:	b8 bc 0a 00 00       	mov    $0xabc,%eax
 7c9:	c7 05 c0 0a 00 00 00 	movl   $0x0,0xac0
 7d0:	00 00 00 
